...
/*
   ...
   à partir des scores par thème de l'utilisateur : ce sont ses vrais
   thèmes, pas un texte générique.

   Si la page est atteinte sans audit (lien direct, résultat partagé), on
   retombe sur les listes indicatives calées sur le score.
 */
...
export function ResultsAssessment({
  ...
  const hasRealData = strengths.length > 0 || weaknesses.length > 0
          ...
          {hasRealData ? (
            strengths.length > 0 ? (
              strengths.map((strength) => (
                <li key={strength.category} className="flex items-start gap-2">
                  <Check className="w-3.5 h-3.5 text-[color:var(--sc-success)] mt-0.5 shrink-0" />
                  <span>
                    {strength.category}
                    <span className="text-[color:var(--sc-text-muted)]"> — {strength.score}%</span>
                  </span>
                </li>
              ))
            ) : (
              <li className="text-[color:var(--sc-text-muted)]">
                Aucun thème n&apos;atteint encore le niveau attendu. C&apos;est justement
                ce que les recommandations ci-dessous vont t&apos;aider à changer.
              </li>
            )
          ...
          {hasRealData ? (
            weaknesses.length > 0 ? (
              weaknesses.map((weakness) => (
                <li key={weakness.category} className="flex items-start gap-2">
                  <AlertTriangle className="w-3.5 h-3.5 text-[color:var(--sc-warn)] mt-0.5 shrink-0" />
                  <span>
                    {weakness.category}
                    <span className="text-[color:var(--sc-text-muted)]"> — {weakness.score}%</span>
                  </span>
                </li>
              ))
            ) : (
