...
export default function Navbar({ isLoggedIn: isLoggedInProp, onLoginClick, onSignupClick }: NavbarProps) {
  ...
  // The "Voir tous les audits" shortcut is only shown on the homepage during
  // this initial rollout. The real homepage (when logged in) lives at /accueil.
  const isHomepage = pathname === "/accueil"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 flex items-center h-16 gap-2">
        ...
        >
          ...
          <span
            aria-hidden="false"
            className="text-lg font-bold tracking-tight text-[color:var(--sc-text)] font-display
              transition-opacity duration-200 group-hover:opacity-0 group-focus-visible:opacity-100"
          >
        ...
        <div className="flex items-center gap-1.5 shrink-0 ml-auto">
          ...
          {isLoggedIn ? (
            <>
              <Link
                ...
                className="hidden sm:inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium
                  text-[color:var(--sc-text-2)] hover:text-[color:var(--sc-blue)] hover:bg-[color:var(--sc-bg-soft)] transition-colors"
                onClick={closeMenus}
              >
            ...
            <>
              <button
                type="button"
                className="hidden sm:inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium
                  text-[color:var(--sc-text-2)] hover:text-[color:var(--sc-blue)] hover:bg-[color:var(--sc-bg-soft)] transition-colors cursor-pointer"
          ...
          {/* Mobile burger */}
          <button
            className="lg:hidden p-2 rounded-lg text-[color:var(--sc-text-2)] hover:text-[color:var(--sc-blue)] hover:bg-[color:var(--sc-bg-soft)] transition-colors"
      ...
      {/* Mobile menu drawer */}
      {mobileOpen && (
        <div className="lg:hidden border-t border-[color:var(--sc-border)] bg-[color:var(--sc-surface)] max-h-[80vh] overflow-y-auto">
          ...
          {!isLoggedIn && (
            <div className="p-4 border-t border-[color:var(--sc-border)] flex items-center gap-3">
              <button
                type="button"
                className="text-sm font-medium text-[color:var(--sc-blue)] hover:underline cursor-pointer"
                onClick={handleLoginClick}
              >
                {t("nav.connexion")}
              </button>
              ...
              <button
                type="button"
                className="text-sm font-medium text-[color:var(--sc-text-2)] hover:text-[color:var(--sc-blue)] cursor-pointer"
          ...
          {isLoggedIn && (
            <div className="p-4 border-t border-[color:var(--sc-border)] flex items-center gap-3">
              <Link
                href="/compte"
                onClick={closeMenus}
                className="text-sm font-medium text-[color:var(--sc-text-2)] hover:text-[color:var(--sc-blue)]"
              >
                {t("nav.monProfil")}
              </Link>
              <span className="text-[color:var(--sc-text-muted)]">·</span>
              <button
                ...
                className="text-sm font-medium text-[color:var(--sc-text-2)] hover:text-white hover:bg-[linear-gradient(180deg,#F87171_0%,#DC2626_100%)] px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
              >
                {t("nav.deconnexion")}
              </button>
            </div>
