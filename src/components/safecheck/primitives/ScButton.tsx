...
type ScButtonVariant = "primary" | "secondary" | "ghost" | "danger" | "destructive"
type ScButtonSize = "sm" | "md" | "lg"
...
export type ScLinkButtonProps = Omit<
...
const sizeClasses: Record<ScButtonSize, string> = {
...
const variantClasses: Record<ScButtonVariant, string> = {
  primary:
...
export function scButtonClassName({
  variant = "primary",
  size = "md",
  className,
  disabled = false,
...
export function ScButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={scButtonClassName({ variant, size, className, disabled })}
    >
      {children}
    </button>
  )
...
export function ScLinkButton({
  ...
  return (
    <Link
      {...linkProps}
      className={scButtonClassName({ variant, size, className })}
    >
      {children}
    </Link>
  )
