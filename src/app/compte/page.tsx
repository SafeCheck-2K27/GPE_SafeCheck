...
export default function MonComptePage() {
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...prev,
      pseudo: auth.user!.pseudo || prev.pseudo,
      email: auth.user!.email || prev.email,
      prenom: auth.user!.prenom || prev.prenom,
      nom: auth.user!.nom || prev.nom,
      pays: auth.user!.pays || prev.pays,
    ...
    <div className="min-h-screen flex flex-col bg-[color:var(--sc-bg)] font-sans">
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-4 md:px-6 py-8">
            ...
            <div className="flex items-center gap-5 shrink-0">
              <div className="relative">
                ...
                <div className="relative w-24 h-24 rounded-full p-[3px] bg-[linear-gradient(135deg,#3B82F6,#6366F1_50%,#06B6D4)] shadow-[0_18px_40px_-14px_rgba(37,99,235,0.50)]">
                  <div className="w-full h-full rounded-full sc-avatar-gradient flex items-center justify-center
                    text-white font-display text-3xl font-bold
                ...
                <button
                  ...
                  className="absolute bottom-0 right-0 w-8 h-8 rounded-full flex items-center justify-center text-white
                    ...
                    border-2 border-[color:var(--sc-surface)]
                    shadow-[0_6px_14px_-4px_rgba(37,99,235,0.55)]
        ...
        <div className="mb-6 sticky top-16 z-30 -mx-4 md:mx-0 px-4 md:px-0">
          <div className="rounded-xl border border-[color:var(--sc-border)] bg-[color:var(--sc-surface)]/85 backdrop-blur-md
            supports-[backdrop-filter]:bg-[color:var(--sc-surface)]/70
            shadow-[var(--sc-shadow-sm)] p-1 flex items-center gap-1 overflow-x-auto">
            {tabs.map((t) => {
              const Icon = t.icon
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[color:var(--sc-text)]/40 backdrop-blur-sm px-4"
          ...
          <div
            className="w-full max-w-sm rounded-2xl p-6 sc-fade-in
...
function StatPill({
  ...
  return (
    <div className="flex flex-col items-center justify-center px-3 py-2.5 rounded-xl
      bg-[color:var(--sc-surface)] border border-[color:var(--sc-border)]
      shadow-[var(--sc-shadow-sm)] min-w-[68px]">
...
function SectionCard({
  ...
  return (
    <section className="rounded-2xl overflow-hidden border border-[color:var(--sc-border)] bg-[color:var(--sc-surface)] shadow-[var(--sc-shadow-sm)]">
      <header className="flex items-start justify-between gap-4 px-5 md:px-6 py-4
        bg-[color:var(--sc-bg-soft)] border-b border-[color:var(--sc-border)]">
        <div className="flex items-start gap-3 min-w-0">
          {Icon && (
            <span className="shrink-0 inline-flex items-center justify-center w-9 h-9 rounded-lg
              bg-[linear-gradient(135deg,var(--sc-blue-soft),var(--sc-blue))] text-white
          ...
          >
                    ...
                    <div key={i} className="flex flex-col items-center gap-1">
                      <span
                        className={`w-2 h-2 rounded-full
                          ${lv.current
                            ? "bg-[color:var(--sc-blue)] ring-4 ring-[color:var(--sc-blue)]/20"
                            : lv.reached
                              ? "bg-[color:var(--sc-success)]"
                              : "bg-[color:var(--sc-border-strong)]"
                          }`}
                      />
                      <span
            ...
            <div className="rounded-xl p-4 border border-[color:var(--sc-border)] bg-[color:var(--sc-bg-soft)]/50">
              <div className="flex items-start gap-3">
                <span className="shrink-0 inline-flex items-center justify-center w-9 h-9 rounded-lg
                  bg-[color:var(--sc-surface)] border border-[color:var(--sc-blue)]/30 text-[color:var(--sc-blue)]
      ...
      >
        <div className="flex gap-5 flex-wrap">
          {form.badges.map((badge, idx) => {
            ...
            return (
              <div key={badge.id} className="flex items-center gap-3 px-3 py-2 rounded-xl
                bg-[color:var(--sc-bg-soft)]/60 border border-[color:var(--sc-border)]">
                <div
                  className={`w-11 h-11 rounded-full flex items-center justify-center text-white text-sm font-bold
                    ring-2 ring-[color:var(--sc-surface)]
                    shadow-[0_8px_18px_-6px_rgba(15,23,42,0.20)] ${tones[idx % tones.length]}`}
                  title={badge.desc}
                >
                  {badge.id}
                </div>
          ...
          {/* Locked badges */}
          {[1, 2].map((n) => (
            <div key={`lock-${n}`} className="flex items-center gap-3 px-3 py-2 rounded-xl
              bg-[color:var(--sc-surface-2)]/50 border border-dashed border-[color:var(--sc-border-strong)]">
              <div className="w-11 h-11 rounded-full flex items-center justify-center
                bg-[color:var(--sc-surface)] border border-[color:var(--sc-border)] text-[color:var(--sc-text-muted)]">
        ...
        <ul className="divide-y divide-[color:var(--sc-border)]">
          {mockGuides.map((g) => (
            <li key={g.id} className="flex items-center justify-between gap-3 py-3 first:pt-0 last:pb-0">
              <div className="flex items-start gap-3 min-w-0">
                <span className="shrink-0 inline-flex items-center justify-center w-8 h-8 rounded-lg
                  bg-[color:var(--sc-bg-soft)] text-[color:var(--sc-blue)] border border-[color:var(--sc-border)]">
...
function AuditRow({ audit }: { audit: typeof mockAudits[number] }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-xl border border-[color:var(--sc-border)] bg-[color:var(--sc-surface)] p-3.5
      hover:border-[color:var(--sc-border-strong)] hover:shadow-[var(--sc-shadow-sm)] transition-all">
      <div className="flex items-start gap-3 min-w-0">
        <span className={`shrink-0 inline-flex items-center justify-center w-9 h-9 rounded-lg
          ${audit.status === "termine"
            ? "bg-[rgba(16,185,129,0.10)] text-[color:var(--sc-success)] border border-[rgba(16,185,129,0.25)] dark:bg-[rgba(16,185,129,0.14)] dark:text-[#6EE7B7] dark:border-[rgba(16,185,129,0.32)]"
  ...
  return (
    <div className="grid gap-6 sc-fade-in">
      ...
      <section className="rounded-2xl overflow-hidden border border-[color:var(--sc-danger)]/30 bg-[color:var(--sc-surface)] shadow-[var(--sc-shadow-sm)]">
        <header className="flex items-start justify-between gap-4 px-5 md:px-6 py-4
          bg-[rgba(239,68,68,0.06)] border-b border-[color:var(--sc-danger)]/20">
          <div className="flex items-start gap-3">
            <span className="shrink-0 inline-flex items-center justify-center w-9 h-9 rounded-lg
              bg-[rgba(239,68,68,0.12)] text-[color:var(--sc-danger)] border border-[color:var(--sc-danger)]/25">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <span className="inline-flex items-center justify-center w-10 h-10 rounded-lg
              bg-[color:var(--sc-bg-soft)] border border-[color:var(--sc-border)] text-[color:var(--sc-blue)]">
  ...
  return (
    <button
      ...
      className={`text-left rounded-xl border p-4 transition-all duration-200 cursor-pointer
        ${active
          ? "border-[color:var(--sc-blue)] bg-[color:var(--sc-bg-soft)] shadow-[var(--sc-shadow-blue-sm)] ring-1 ring-[color:var(--sc-blue)]/30"
          : "border-[color:var(--sc-border)] bg-[color:var(--sc-surface)] hover:border-[color:var(--sc-blue)]/55 hover:-translate-y-0.5 hover:shadow-[var(--sc-shadow)]"
        }
      `}
      aria-pressed={active}
    >
      <div className="flex items-center gap-2.5 mb-2">
        <span
          className={`inline-flex items-center justify-center w-9 h-9 rounded-lg
            ${active
              ? "bg-[linear-gradient(135deg,var(--sc-blue-soft),var(--sc-blue))] text-white shadow-[var(--sc-shadow-blue-sm)]"
              : "bg-[color:var(--sc-bg-soft)] text-[color:var(--sc-blue)] border border-[color:var(--sc-border)]"
            }
          `}
        >
        ...
        {active && (
          <span className="ml-auto inline-flex items-center justify-center w-5 h-5 rounded-full
            bg-[color:var(--sc-blue)] text-white">
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-3.5 border-b border-[color:var(--sc-border)] last:border-b-0">
      ...
      <button
        ...
        className={`shrink-0 relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 cursor-pointer
          focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--sc-blue)]/45 focus-visible:ring-offset-2 focus-visible:ring-offset-[color:var(--sc-surface)]
          ${checked
            ? "bg-[linear-gradient(180deg,var(--sc-blue-soft),var(--sc-blue))] shadow-[var(--sc-shadow-blue-sm)]"
            : "bg-[color:var(--sc-surface-2)] border border-[color:var(--sc-border-strong)]"
          }
        `}
      >
        <span
          className={`inline-block h-5 w-5 rounded-full bg-white shadow-[0_2px_6px_rgba(15,23,42,0.20)] transition-transform duration-200
...
function ExportTile({ title, desc }: { title: string; desc: string }) {
  ...
  return (
    <div className="rounded-xl border border-[color:var(--sc-border)] bg-[color:var(--sc-surface)] p-4
      hover:border-[color:var(--sc-blue)]/45 hover:shadow-[var(--sc-shadow)] hover:-translate-y-0.5 transition-all">
